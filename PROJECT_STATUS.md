# MON-T-BLASTER — Project Status

## 🎮 Live URL
https://mindsentrix-cpu.github.io/mon-t-blaster/

## 📌 Current Version
v2.2

## ✅ Completed
- [2026-03-22] v1.0 — Initial project structure
- [2026-03-22] index.html — Full game uploaded (HTML5 Canvas)
- [2026-03-22] assets/sprites/ and assets/audio/ folders created
- [2026-03-22] src/game.js placeholder created
- [2026-03-22] GitHub Pages enabled
- [2026-03-22] v1.1 — Refactored: HTML/CSS/JS split into separate files
- [2026-03-22] assets/css/style.css created (all styles)
- [2026-03-22] assets/js/game.js created (all game logic)
- [2026-03-22] WORKFLOW.md added (development conventions)
- [2026-03-22] v1.2 — balance: lowered base projectile speed from 12 to 6, increased upgrade steps from 4 to 6 — speed upgrades now feel meaningfully different
- [2026-03-22] v1.3 — fix: main menu fully responsive for iPhone (safe areas, 100dvh, viewport-fit=cover, clamp spacing)
- [2026-03-22] v1.4 — balance: projectile base speed lowered from 6→3, upgrade steps 6→4 (max 15)

## 🔄 In Progress
- Nothing currently

## 📋 Backlog
- [ ] Separate JS into src/game.js
- [ ] Add real sprite PNGs from Leonardo AI
- [ ] Add audio assets
- [ ] Power-ups system (double shot, slow motion, shield)
- [ ] Sound effects
- [ ] High score persistence (localStorage)
- [ ] Mobile PWA support
- [ ] App Store / Google Play (Godot migration)

## 📝 Commit Log
| Date | Version | Description |
|------|---------|-------------|
| 2026-03-22 | v1.0 | Initial structure + full game upload |
| 2026-03-22 | v1.0 | Add PROJECT_STATUS.md — change tracking initialized |
| 2026-03-22 | v1.1 | Refactor: split HTML/CSS/JS + add WORKFLOW.md |
| 2026-03-22 | v1.2 | Balance: projectile speed rebalance for better upgrade feel |
| 2026-03-22 | v1.3 | Fix: main menu responsive for iPhone — safe areas, dvh, clamp |
| 2026-03-22 | v1.4 | Balance: projectile speed base 6→3, steps 6→4 |

---
**Rule: Update this file on every single commit going forward.**
Every entry must include: date, version bump, and description of changes.
